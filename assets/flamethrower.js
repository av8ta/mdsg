// flamethrower. MIT. https://github.com/fireship-io/flamethrower
...
function y (o) {
  let e
  if (o.altKey || o.ctrlKey || o.metaKey || o.shiftKey) { return { type: 'disqualified' } }
  for (let t = o.target; t.parentNode; t = t.parentNode) {
    if (t.nodeName === 'A') {
      e = t
      break
    }
  }
  if (e && e.host !== location.host) { return e.target = '_blank', { type: 'external' } }
  if (e && 'cold' in (e == null ? void 0 : e.dataset)) { return { type: 'disqualified' } }
  if (e != null && e.hasAttribute('href')) {
    const r = e.getAttribute('href'); const n = new URL(r, location.href)
    if (o.preventDefault(), r != null && r.startsWith('#')) { return g(r), { type: 'scrolled' } }
    const s = l(n.href); const i = l()
...
function E (o) {
  const e = (i) => Array.from(i.querySelectorAll('head>:not([rel="prefetch"]')); const t = e(document); const r = e(o); const { staleNodes: n, freshNodes: s } = k(t, r)
...
function p (o) {
  const e = document.createElement('script'); const t = Array.from(o.attributes)
  for (const { name: r, value: n } of t) { e[r] = n }
...
class A {
  constructor (e) {
    this.opts = e, this.enabled = !0, this.prefetched = /* @__PURE__ */ new Set(), this.opts = { ...F, ...e != null ? e : {} }, window != null && window.history
      ? (document.addEventListener('click', (t) => this.onClick(t)), window.addEventListener('popstate', (t) => this.onPop(t)), this.prefetch())
      : (console.warn(
  ...
  go (e) {
    const t = window.location.href; const r = new URL(e, location.origin).href
  ...
  prefetch () {
    if (this.opts.prefetch === 'visible') { this.prefetchVisible() } else if (this.opts.prefetch === 'hover') { this.prefetchOnHover() } else {}
  }
  ...
  async reconstructDOM ({ type: e, next: t, prev: r }) {
    ...
    try {
      if (this.log('\u26A1', e), ['popstate', 'link', 'go'].includes(e) && t !== r) {
        ...
        const s = await (await fetch(t, { headers: { 'X-Flamethrower': '1' } })).text(); const i = v(s)
        E(i), this.opts.pageTransitions && document.createDocumentTransition
